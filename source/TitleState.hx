...
class TitleState extends MusicBeatState
{
	...
	var ngSpr:FlxSprite;
	public var rickroll:Bool = false;
	...
	override public function create():Void
	{
		...
		#else
		if(!FlashingState.leftState && !FlashingState.rickroll) {
		...
		{
			rickroll = true;
			FlxG.switchState(RickrollState.new);
		}
		else if (FlashingState.leftState && !FlashingState.rickroll)
		{
			if (initialized)
				startIntro();
			else
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					PlayState.SONG = Song.loadFromJson('april-fools', 'april-fools');
